...
func (r *ServiceRoleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Update ServiceRole status with the actual ServiceAccount name
	serviceRole.Status.ServiceAccountName = serviceAccountName
	if err := r.Status().Update(ctx, &serviceRole); err != nil {
		logger.Error(err, "Failed to update ServiceRole status")
		return ctrl.Result{}, err
	}

	logger.Info("ServiceRole reconciled successfully",
